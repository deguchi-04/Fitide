...
const { weightHistory, measurementDomain } = await load('../lib/progress-data.ts');
const weights = [{date:'2026-09-19',weightKg:79}, {date:'2026-01-01',weightKg:80}, {date:'2027-01-01',weightKg:75}];
assert.deepEqual(weightHistory(weights, '2026-09-20').map(item => item.weightKg), [80,79]);
assert.equal(weights[0].weightKg,79);
assert.deepEqual(measurementDomain([80,79]), [77,82]);
assert.deepEqual(measurementDomain([80]), [78,82]);
assert.deepEqual(weightHistory([], '2026-09-20'), []);
console.log('Weight history: chronological, all recorded points, adaptive scales and empty state passed.');
